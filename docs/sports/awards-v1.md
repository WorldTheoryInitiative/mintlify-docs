...
The Awards API v1 provides real-time access to sports award markets (MVP, Rookie of the Year, etc.) with odds from multiple prediction market platforms. This endpoint aggregates award betting data across Polymarket, Kalshi, ProphetX, and Novig without database dependencies.
...
- **Multi-Platform Aggregation** - Odds from 5 platforms: Polymarket, Kalshi, ProphetX, Novig, and SX.bet
...
- **Award Filtering** - Filter by award type (MVP, OPOY, DPOY, Super Bowl winner, etc.)
...
**Note**: SX.bet is integrated but currently has no active championship markets. The API will query SX.bet but typically return 0 candidates until markets become active.
...
- **Super Bowl** - Super Bowl LX Winner
  - Polymarket: 33 teams | Kalshi: 32 teams
...
| `award_type` | string | No | - | Filter by award type: `mvp`, `opoy`, `dpoy`, `oroy`, `droy`, `cpoy`, `coy`, `superbowl` |
| `season` | number | No | `2025` | Season year |
| `platform` | string | No | - | Filter by platforms (comma-separated): `polymarket`, `kalshi`, `prophetx`, `sxbet` |
...
### Get All NFL Awards
```bash
curl -X GET "https://ptahricowhzsxfhqnvqe.supabase.co/functions/v1/awards-v1?league=nfl" \
...
### Get NFL MVP Only
```bash
curl -X GET "https://ptahricowhzsxfhqnvqe.supabase.co/functions/v1/awards-v1?league=nfl&award_type=mvp" \
  -H "X-API-Key: your-api-key" \
  -H "Authorization: Bearer your-supabase-token"
```

### Get Awards with Decimal Odds
```bash
curl -X GET "https://ptahricowhzsxfhqnvqe.supabase.co/functions/v1/awards-v1?league=nfl&award_type=mvp&odds_format=decimal" \
  -H "X-API-Key: your-api-key" \
  -H "Authorization: Bearer your-supabase-token"
```
...
### Filter by Platform
```bash
curl -X GET "https://ptahricowhzsxfhqnvqe.supabase.co/functions/v1/awards-v1?league=nfl&platform=kalshi" \
  -H "X-API-Key: your-api-key" \
  -H "Authorization: Bearer your-supabase-token"
```
...
Returns data from multiple platforms in a single response:
...
**Note**: The response shows **two award objects** with the same ID - one for each platform. This allows you to compare odds across platforms for the same award.
...
```json
{
  "error": {
    ...
    "details": {
      "validation_errors": {
        "award_type": ["Invalid award_type. Valid options: mvp, opoy, dpoy, oroy, droy, cpoy, coy, superbowl"]
      }
    },
    "timestamp": "2025-10-12T20:00:00.000Z"
  }
...
### 1. Filter by Award Type

Request only the awards you need to reduce response size:
...
- **NEW**: SX.bet platform integration for championship markets
- **NEW**: League IDs configured for Super Bowl (10397), AFC Champion (10398), NFC Champion (10399), and Super Bowl MVP (10407)
- **NOTE**: SX.bet leagues exist but currently have 0 active markets
- **IMPROVED**: Platform coverage now includes 5 platforms (Polymarket, Kalshi, ProphetX, Novig, SX.bet)
- **IMPROVED**: Documentation includes SX.bet player prop league information
